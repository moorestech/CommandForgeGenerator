...
namespace CommandForgeGenerator.Generator.NameResolve;
...
public static class NameResolver
{
    public static NameTable Resolve(Semantics semantics, SchemaTable schemaTable)
    {
        ...
        foreach (var typeId in typeNames.Keys)
        {
            ...
            //一応残しておく nameSpaces[typeId] = $"CommandForgeGenerator{string.Join("", ((IEnumerable<string>)parentNames).Reverse().Select(n => $".{n}Module"))}";
    ...
    {
        var ifThenSchema = switchSchema.IfThenArray.ToDictionary(ifThen => schemaTable.Table[ifThen.Schema])[typeSemantics.Schema];
        return $"{ifThenSchema.When}{switchSchema.PropertyName?.ToCamelCase()}";
    }

    public static string GetModelName(this TypeName typeName)
    {
        return $"CommandForgeGenerator.Model.{typeName.ModuleName}.{typeName.Name}";
    }

    public static string GetLoaderName(this TypeName typeName)
    {
        return $"CommandForgeGenerator.Loader.{typeName.ModuleName}.{typeName.Name}Loader";
    }
