...
public static class LoaderGenerator
{
    ...
    private static (string fileName, string code) GenerateGlobalLoaderCode(Semantics semantics, NameTable nameTable)
    {
        ...
        return (
            "CommandForgeGenerator.loader.g.cs",
            $$$"""
               namespace CommandForgeGenerator.Loader
               {
                               ...
                               semantics
                                   ...
                                   .Select(root => {
                                           ...
                                           return $$$"""
                                                     ...
                                                     {
                                                         // code
                                                         return CommandForgeGenerator.Loader.{{{name.ModuleName}}}.{{{name.Name}}}Loader.Load(json);
                                                     }
                                                     catch
                                                     {
                                                     }
    ...
    private static (string fileName, string code) GenerateTypeLoaderCode(TypeDefinition typeDefinition, Semantics semantics, NameTable nameTable)
    {
        ...
        return (
            $"CommandForgeGenerator.loader.{typeDefinition.TypeName.ModuleName}.{typeDefinition.TypeName.Name}.g.cs",
            $$$"""
               namespace CommandForgeGenerator.Loader.{{{typeDefinition.TypeName.ModuleName}}}
               {
            ...
            builder.AppendLine(
                $$$$"""
                    if (json["{{{{jsonPropertyName}}}}"] == null)
                    {
                        ...
                        throw new global::CommandForgeGenerator.Loader.CommandForgeGeneratorLoaderException(propertyPath, typeof({{{{typeName}}}}).Name, "{{{{propertyDefinition.PropertyName}}}}");
                    }
    ...
    private static (string fileName, string code) GenerateSwitchLoaderCode(SwitchId switchId, Semantics semantics, NameTable nameTable)
    {
        ...
        return (
            $"CommandForgeGenerator.loader.{name.ModuleName}.{name.Name}.g.cs",
            $$$"""
               namespace CommandForgeGenerator.Loader.{{{name.ModuleName}}}
               {
                   public static class {{{name.Name}}}Loader
                   {
                       public static global::CommandForgeGenerator.Model.{{{name.ModuleName}}}.{{{name.Name}}} Load(global::Newtonsoft.Json.Linq.JToken json)
                       {
    ...
    private static string GetLoaderName(Type type)
    {
        return $"{
            type switch
            {
                BooleanType booleanType => "CommandForgeGenerator.Loader.BuiltinLoader.LoadBoolean",
                FloatType floatType => "CommandForgeGenerator.Loader.BuiltinLoader.LoadFloat",
                IntType intType => "CommandForgeGenerator.Loader.BuiltinLoader.LoadInt",
                StringType stringType => "CommandForgeGenerator.Loader.BuiltinLoader.LoadString",
                UUIDType uuidType => "CommandForgeGenerator.Loader.BuiltinLoader.LoadUUID",
                Vector2IntType vector2IntType => "CommandForgeGenerator.Loader.BuiltinLoader.LoadVector2Int",
                Vector2Type vector2Type => "CommandForgeGenerator.Loader.BuiltinLoader.LoadVector2",
                Vector3IntType vector3IntType => "CommandForgeGenerator.Loader.BuiltinLoader.LoadVector3Int",
                Vector3Type vector3Type => "CommandForgeGenerator.Loader.BuiltinLoader.LoadVector3",
                Vector4Type vector4Type => "CommandForgeGenerator.Loader.BuiltinLoader.LoadVector4",
    ...
    public static string GenerateLoaderExceptionTypeCode()
    {
        return """"
               namespace CommandForgeGenerator.Loader
               {
                   public class CommandForgeGeneratorLoaderException : global::System.Exception
                   {
                       public string ErrorProperty;
                       public string ErrorType;
                       public string PropertyPath;
                   
                       public CommandForgeGeneratorLoaderException(string propertyPath, string errorType, string errorProperty)
                       {
